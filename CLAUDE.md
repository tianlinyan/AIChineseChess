...
- **`pikafish.py`** — `PikafishEngine`: UCI protocol wrapper for the Pikafish NNUE engine (Stockfish-derived, master-level). Async search via daemon thread + `pyqtSignal` relay to main thread. Engine stdout is pumped by a dedicated reader thread into a line queue — all reads are deadline-bounded `queue.get(timeout=)` (a silently-hung engine can no longer deadlock the game or window close); close/kill terminate the process before taking the lock. Falls back silently to MCTS if binary not found. Binary goes in `engines/pikafish.exe` or set `PIKAFISH_PATH` env var. **UCI coordinate conversion**: internal row = 9 − UCI rank (Pikafish rank 0 = red's back rank = internal row 9). **FEN**: standard orientation (row 0 = black's back rank), no row reversal — verified against `position startpos` + `d` command. Before each `position` command, `_purge_lines()` drains stale `bestmove` from the line queue (prevents reading a previous search's move as the current result). Reader thread uses UTF-8 with `errors='replace'` and sets `_reader_dead` flag on exit so `_read_line` fails fast instead of waiting for timeout.
- **`egtb.py`** — Endgame tablebase: local heuristic for ≤10 pieces (`EGTB_MAX_PIECES`) with defender advisor/bishop awareness (lone rook vs full guard = draw, lone knight beats bare king, double cannons beat bare king, single pawn wins only vs bare king when crossed and not on the back rank). Optional chessdb.cn cloud probe for ≤6-piece endgames (`EGTB_CLOUD_MAX_PIECES`, `chessdb.php?action=queryall` API — pipe-delimited text, outcome parsed from the note field `(W/D/L-M-NNNN)`). Positive cache 5-min TTL (5000 cap) + negative cache 60s (5000 cap) + circuit breaker (120s after 3 consecutive network failures). `probe(..., allow_cloud=False)` is mandatory inside search/MCTS leaves — no synchronous HTTP in the search loop. NOTE: the cloud path is implemented and verified but currently not wired into production (all production callers use `allow_cloud=False`).
- **`openings.py`** — 17 standard opening lines (中炮对屏風馬, 顺炮, etc.), weighted random selection by prefix-matching move history.
...
- **`constants.py`** — Board size, search config, MCTS config, AI timeout/retry settings, vision mode config. Utility functions: `format_duration`, `format_coord`, `parse_coord`, `format_move`. Note: `SEARCH_MAX_DEPTH` is the default for the UI 搜索强度 spinbox (1~6) — it is **not** Alpha-Beta depth; it scales MCTS simulations and Pikafish time (see controller `_DEPTH_SIMS_MAP`). `AI_CONNECT_TIMEOUT` (15s) is separate from `AI_TIMEOUT_SECONDS` (300s) — connect timeout fast-fails on unreachable endpoints; read timeout tolerates long-thinking models.
...
- **`controller.py`** — `GameController`: central state machine. Three AI modes: `hybrid` (default), `search_only`, `llm_only`. Manages Pikafish async lifecycle, version gating, retry logic (non-retryable errors skip retries), fallback chain. Key subsystems:
  ...
  - **Background engine lifecycle**: `_stop_background_engines()` cancels in-flight Pikafish (`stop()` → UCI `stop` command) and MCTS (`_active_mcts.stop()`) on reset/pause/shutdown — releases CPU immediately and prevents old search results from blocking new searches. MCTS thread joined with 2s timeout on shutdown.
...
- **Vision mode**: When enabled, board is rendered to QPixmap via `render()` (not `grab()`) and sent as JPEG base64 to multimodal models. Dual-channel prompt: image for strategic perception, text legal-move list for tactical execution. **DeepSeek API does NOT support `image_url` content type** (text-only) — controller auto-disables vision mode for `model.type == 'deepseek'`; `AIWorker._build_user_content()` has a safety net that filters `image_url` for DeepSeek models.
- **Opening book**: 17 standard opening lines cover the first 4-6 half-moves, saving tokens and ensuring strong opening play.
- **EGTB integration**: local heuristic for ≤10 pieces; chessdb.cn cloud probe (queryall API) implemented and verified for ≤6-piece endgames but currently not wired into production (5-min positive cache, 60s negative cache, circuit breaker — all production callers use `allow_cloud=False`). Search/MCTS leaves use `allow_cloud=False` — no synchronous HTTP inside the search loop (it used to paralyze endgame search).
