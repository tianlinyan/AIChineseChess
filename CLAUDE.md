...
测试脚本：
```bash
python tests/smoke_engine.py      # 无 GUI 冒烟
python tests/compare_movegen.py   # 走法生成基线对拍
```

## Architecture

五层依赖链：`domain/ → ai/ → app/ → ui/ → main.py`，上层从下层导入，永不反向。
...
- **Always tool calling**: 无 UI 开关。提示词指导函数调用。文本解析器仅作为兜底。
...
- **Opening book**: 17 条标准开局线（现已扩充至 46 条），前缀加权随机选择。
