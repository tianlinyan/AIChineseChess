...
- 隐藏层1：512 神经元，ReLU（训练）/ ClippedReLU（推理）
- 隐藏层2：64 神经元，ReLU / ClippedReLU
...
class NnueNet:
    ...
    def evaluate_incremental(self, acc: 'Accumulator') -> float:
        """增量评估（使用累加器，跳过特征提取和 input→hidden 矩阵乘）。

        累加器维护 h1 = Σ w1[active_features] + b1 的运行和。
        走子时增量更新累加器，评估时只需 hidden1→hidden2→output。
        """
        if not self._loaded or acc is None:
            return 0.0

        hidden1 = np.clip(acc.values, 0, QA)
        hidden2 = np.clip(np.dot(hidden1, self._w2) + self._b2, 0, QA)
        score = float(np.dot(hidden2, self._w3) + self._b3)
        return score * 100.0

    # ── 累加器 ──

    def create_accumulator(self, board: list) -> 'Accumulator':
        """从棋盘创建累加器（全盘扫描初始化）。"""
        if not self._loaded:
            return None
        acc = Accumulator(HIDDEN1_DIM)
        features = self.extract_features(board)
        # h1 = features @ w1 + b1
        # 等价于：h1 = sum(w1[active_indices]) + b1
        active = np.where(features > 0)[0]
        for idx in active:
            acc.values += self._w1[idx]
        acc.values += self._b1
        return acc
...
class Accumulator:
    ...
    维护 h1 = Σ w1[active_features] + b1 的运行和。
    走子时增量更新（O(1)），避免每次全盘特征提取+矩阵乘。
    """
    __slots__ = ('values',)
