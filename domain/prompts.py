...
| 棋子 | 价值 | 走法 | 关键约束 |
|------|------|------|---------|
| 将 K / 将 k | ∞ | 九宫内 1 格 | 禁出九宫；禁双方将同列无遮挡 |
| 士 A / 士 a | 2 | 九宫内斜 1 格 | 禁出九宫 |
| 相 B / 相 b | 2 | 田字对角(横2竖2) | 禁过河；相眼有子不能走 |
| 馬 N / 馬 n | 4 | 日字(横1竖2或横2竖1) | 蹩脚方向有子不能走 |
| 車 R / 車 r | 9 | 直线任意格，不可越子 | — |
| 炮 C / 炮 c | 4.5 | 移动=直线任意格；吃子=隔1子 | 移不行越子，吃必须隔子 |
| 卒 P / 卒 p | 1→3 | 未过河=只前进；过河=前左右各1格 | 永不后退；过河=红卒行≤5,黑卒行≥6 |

**兑子原则**：低价值换高价值。马(4)换车(9)血赚，炮(4.5)换马(4)微优。**决不**用车换马/炮（除非有杀棋）。残局：炮贬值(缺架)，马升值(少蹩脚)，过河卒大幅升值。
...
4. **走闲棋** — 来回移动同一子、士相在九宫内无意义挪动。**每步必须有明确目的**
...
| 手段 | 車占肋道(D/F列)，马跳活位，炮架中路 | 捉双/抽将/闪击/牵制/卧槽馬 | 車控要线，将可助攻，卒逼宫 |
| 禁忌 | 重复动子、车未出先走边卒 | 贪吃弃子失先手 | 炮缺架优先用车马 |

# 五、思考流程

**1. 安全** — 我被将军？→ 只考虑应将（躲将/垫子/吃子）。对方上一步意图？我的将暴露吗？
**2. 机会** — 能将军/吃大子/捉双/抽将/闪击吗？候选→对方最强应对→我能否应对？杀棋路线：卧槽馬、铁门栓、重炮、双車错。**优势时简化局面（兑子），劣势时制造复杂（保留变化）。**
...
**屏風馬应对**：红 H8→E8 后，黑马 B1→C3 保中卒同时出动子力。
...
送将、将帅对面、规则违规（出九宫/相过河/卒后退/車越子/炮吃无架）。
**走闲棋**：来回移动同一子、士相在九宫无意义挪动——每步必须有明确目的。

# 三、估值与兑子

| 车 9 | 炮 4.5 | 马 4 | 相 2 | 士 2 | 过河卒 2~3 | 未过河卒 1 |

低价值换高价值（马换车、炮换车）。不轻易用车换马/炮。残局炮贬值(缺架)、马升值(少蹩脚)、过河卒大幅升值。

# 四、阶段策略

**开局(≤10)**：快速出子，車占肋道(D/F列)，马跳活位，炮架中路。忌重复动子。
**中局(11~25)**：捉双/抽将/闪击/牵制/卧槽馬。同时注意己方安全。
**残局(>25)**：卒逼宫，将可助攻。車控要线最优。
...
**2. 子力得失**（用估值计算）
能吃子？（车9>炮4.5>马4>相/士 2>卒1~3）能捉双/抽将净赚？是否送子？

**3. 局面安全**
走子后己方将是否暴露？是否给对手将军或捉双的机会？存在明显战术漏洞则**直接否决**该候选。

**4. 子力活跃度**
車占要道？马跳好位？炮有炮架？是否改善了关键子力的位置？
...
def build_move_prompt(current_player: int, board_str: str, history: str,
    ...
    if vision_mode:
        ...
        parts.append("从棋盘截图中获取：阵型结构、子力分布、开放线路、将防护、卒推进、潜在威胁。")
        parts.append("棋子识别：红黑同名，仅颜色区分：将(K/k) 士(A/a) 相(B/b) 馬(N/n) 車(R/r) 炮(C/c) 卒(P/p)")
