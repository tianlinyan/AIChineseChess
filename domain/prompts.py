...
SEARCH_BEST_MOVE_TOOL = {
    ...
    "function": {
        ...
        "description": (
            "调用本地 Alpha-Beta 战术搜索引擎分析局面，返回最佳走法与候选列表。"
        ...
        "parameters": {
            "type": "object",
            "properties": {
                "depth": {"type": "integer", "description": "搜索深度 2~8（默认 3）。越深越准但越慢，建议 ≤5。"},
...
**4. 对比引擎**（如有）— "引擎参考走法"来自独立引擎，但引擎间强度差异大，**信赖程度以参考块内的具体说明为准**（"默认采信"或"仅供参考"）。独立分析后对比：一致→采纳；不一致→有具体战术理由（长线弃子/残局过渡/阵型缺陷）则坚持己见。
...
def build_move_prompt(current_player: int, board_str: str, history: str,
    ...
    if vision_mode:
        ...
        parts.append("从图像获取战略意图 → 在列表中确认对应坐标 → 调用 `move_piece`（列表已剔除送将/违规走法，无需再验证）")
    else:
        parts.append(f"## 当前棋盘 — {player_display}({player_color}) [{phase}] 第{move_count}回合")
        parts.append("```")
