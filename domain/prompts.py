...
**长将**：禁止反复将军同一位置。若循环中某一方步步将军，该方直接判负。
...
| 車 9 | 炮 4.5 | 馬 4 | 相/象 2 | 仕/士 2 | 过河兵/卒 2~3 | 未过河兵/卒 1 |
...
**2. 子力得失**（用估值计算）
能吃子？（車9>炮4.5>馬4>相/象、仕/士2>兵/卒1~3）能捉双/抽将净赚？是否送子？
...
def build_move_prompt(current_player: int, board_str: str, history: str,
    ...
    if vision_mode:
        ...
        parts.append("棋子识别：红：帥(K)仕(A)相(B)馬(N)車(R)炮(C)兵(P) | 黑：将(k)士(a)象(b)馬(n)車(r)炮(c)卒(p)")
